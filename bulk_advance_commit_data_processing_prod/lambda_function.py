...
def lambda_handler(event, context):
    ...
    get_records="""select * from forecast_plan where is_processed=0 and active=1"""
    ...
    for index, row in db_data.iterrows():
        ...
        (row.forecast_commit3==0 and row.forecast_commit3_date is None) or (row.forecast_commit4==0 and row.forecast_commit4_date is None)):
            ...
            target_recovery=row.forecast_target_recovery
            commit_date=row.date
            ...
            check = """select id from advance_commit where ori_part_number=%s and prod_week={} and prod_year={} and process_week={} and demand_category='{}'""".format(
            ...
            (row.forecast_commit3!=0 and row.forecast_commit3_date is not None) or (row.forecast_commit4!=0 and row.forecast_commit4_date is not None)):
                sql = """insert into advance_commit(business_unit,ori_part_number,process_week,process_year,prod_week,
                ...
                commit1_accepted,commit2_accepted,commit3_accepted,commit4_accepted,status,target_recovery)
                values(%s,%s,%s,%s,%s,
                %s,%s,%s,%s,%s,
                %s,%s,%s,%s,%s,
                %s,%s,%s,%s,%s,
                ...
                forecast_commit1_accepted,forecast_commit2_accepted,forecast_commit3_accepted,forecast_commit4_accepted,target_recovery]
                ...
                condb(sql, col)
                sql1 = """update forecast_plan set is_processed=1,active=1 where ori_part_number=%s and prod_week={} and prod_year={}""".format(
                prod_week, prod_year)
                ...
                (row.forecast_commit3==0 and row.forecast_commit3_date is None) and (row.forecast_commit4==0 and row.forecast_commit4_date is None):
                    delete_records="""update advance_commit set 
                    ...
                    cm_recommit_status=if(ifnull(recommited,0)=1 and cm_recommit_status='INCOMPLETE','COMPLETE',cm_recommit_status)
                    ...
                    sql = """update advance_commit set 
                    commit1_old_value=commit1,commit1_date_old_value=commit1_date,commit1={},commit1_date=%s,commit1_week=%s,commit1_year=%s,commit1_accepted=%s,commit1_irp_processed=False,
                    commit2_old_value=commit2,commit2_date_old_value=commit2_date,commit2={},commit2_date=%s,commit2_week=%s,commit2_year=%s,commit2_accepted=%s,commit2_irp_processed=False,
                    commit3_old_value=commit3,commit3_date_old_value=commit3_date,commit3={},commit3_date=%s,commit3_week=%s,commit3_year=%s,commit3_accepted=%s,commit3_irp_processed=False,
                    commit4_old_value=commit4,commit4_date_old_value=commit4_date,commit4={},commit4_date=%s,commit4_week=%s,commit4_year=%s,commit4_accepted=%s,commit4_irp_processed=False,kr_processed=0,irp_processed=0,
                    commit_date=%s,cause_code=%s,cause_code_remark=%s,target_recovery=%s,
                    cm_recommit_status=if(ifnull(recommited,0)=1 and cm_recommit_status='INCOMPLETE','COMPLETE',cm_recommit_status)
                    where ori_part_number=%s and prod_year={} and prod_week={} and process_week={} and demand_category='{}'""".format(new_commit1,new_commit2,new_commit3,new_commit4,prod_year,prod_week,process_week,demand_category)
                    ...
                    col = [commit1_date,commit1_week,commit1_year,forecast_commit1_accepted,commit2_date,commit2_week,commit2_year,forecast_commit2_accepted,\
                    commit3_date,commit3_week,commit3_year,forecast_commit3_accepted,commit4_date,commit4_week,commit4_year,forecast_commit4_accepted,\
                    commit_date,cause_code,cause_code_remark,target_recovery,ori_part_number]
                    print(sql)
                    print(commit1_date,commit2_date,commit3_date,commit4_date)
                    condb(sql,col)
                    sql1 = """update forecast_plan set is_processed=1,active=1 where ori_part_number=%s and prod_week={} and prod_year={}""".format(prod_week, prod_year)
                    col1 = [ori_part_number]
                    condb(sql1, col1)
        ...
        (row.gsa_commit3==0 and row.gsa_commit3_date is None) or (row.gsa_commit4==0 and row.gsa_commit4_date is None)):
            ...
            target_recovery=row.gsa_target_recovery
            commit_date=row.date
            ...
            (row.gsa_commit3!=0 and row.gsa_commit3_date is not None) or (row.gsa_commit4!=0 and row.gsa_commit4_date is not None)):
                ...
                new_commit4, commit4_date, commit4_week, commit4_year, created_by, created_at, updated_by,
                updated_at,commit_date,cause_code,cause_code_remark,
                gsa_commit1_accepted,gsa_commit2_accepted,gsa_commit3_accepted,gsa_commit4_accepted,target_recovery]
                ...
                (row.gsa_commit3==0 and row.gsa_commit3_date is None) and (row.gsa_commit4==0 and row.gsa_commit4_date is None):
                    ...
                    where ori_part_number=%s and prod_year={} and prod_week={} and process_week={} and demand_category='{}'""".format(new_commit1,new_commit2,new_commit3,new_commit4,prod_year,prod_week,process_week,demand_category)
                    print("record updated to advance_commit table with ori_part_number", ori_part_number,"prod_week and prod_year", prod_week, prod_year,"for demand category",demand_category)
                    col = [commit1_date,commit1_week,commit1_year,gsa_commit1_accepted,commit2_date,commit2_week,commit2_year,gsa_commit2_accepted,\
                    commit3_date,commit3_week,commit3_year,gsa_commit3_accepted,commit4_date,commit4_week,commit4_year,gsa_commit4_accepted,\
                    commit_date,cause_code,cause_code_remark,target_recovery,ori_part_number]
                    print(sql)
                    print(commit1_date,commit2_date,commit3_date,commit4_date)
                    condb(sql,col)
        ...
        (row.nongsa_commit3==0 and row.nongsa_commit3_date is None) or (row.nongsa_commit4==0 and row.nongsa_commit4_date is None)):
            ...
            target_recovery=row.nongsa_target_recovery
            commit_date=row.date
            ...
            'committed value2:',new_commit2,'committed date2:',commit2_date,'committed value3:',new_commit3,'committed date3:',commit3_date,'\n',\
            'committed value4:', new_commit4,'committed date4:',commit4_date,'cause_code:',cause_code,'cause_code_remark:',cause_code_remark)
            check = """select id from advance_commit where ori_part_number=%s and prod_week={} and prod_year={} and process_week={} and demand_category='{}'""".format(prod_week, prod_year, week_num, demand_category)
            col = [ori_part_number]
            id_found = list(condb(check, col))
            print(id_found, id)
            ...
            (row.nongsa_commit3!=0 and row.nongsa_commit3_date is not None) or (row.nongsa_commit4!=0 and row.nongsa_commit4_date is not None)):
                ...
                commit_date,cause_code,cause_code_remark,
                nongsa_commit1_accepted,nongsa_commit2_accepted,nongsa_commit3_accepted,nongsa_commit4_accepted,target_recovery]
                print("record inserted to advance_commit table with ori_part_number", ori_part_number,"prod_week and prod_year", prod_week, prod_year,"for demand category",demand_category)
                print(sql)
                condb(sql, col)
                sql1 = """update forecast_plan set is_processed=1,active=1 where ori_part_number=%s and prod_week={} and prod_year={}""".format(prod_week, prod_year)
                ...
                print("entered else part nongsa test")
                ...
                (row.nongsa_commit3==0 and row.nongsa_commit3_date is None) and (row.nongsa_commit4==0 and row.nongsa_commit4_date is None):
                    ...
                    where ori_part_number=%s and prod_year={} and prod_week={} and process_week={} and demand_category='{}'""".format(new_commit1,new_commit2,new_commit3,new_commit4,prod_year,prod_week,process_week,demand_category)
                    print("record updated to advance_commit table with ori_part_number", ori_part_number,"prod_week and prod_year", prod_week, prod_year,"for demand category",demand_category)
                    col = [commit1_date,commit1_week,commit1_year,nongsa_commit1_accepted,commit2_date,commit2_week,commit2_year,nongsa_commit2_accepted,\
                    commit3_date,commit3_week,commit3_year,nongsa_commit3_accepted,commit4_date,commit4_week,commit4_year,nongsa_commit4_accepted,\
                    commit_date,cause_code,cause_code_remark,target_recovery,ori_part_number]
                    print(sql)
                    print(commit1_date,commit2_date,commit3_date,commit4_date)
                    condb(sql,col)
        ...
        (row.system_commit3==0 and row.system_commit3_date is None) or (row.system_commit4==0 and row.system_commit4_date is None)):
            ...
            target_recovery=row.system_target_recovery
            commit_date=row.date
            ...
            (row.system_commit3!=0 and row.system_commit3_date is not None) or (row.system_commit4!=0 and row.system_commit4_date is not None)):
                ...
                new_commit4, commit4_date, commit4_week, commit4_year, created_by, created_at, updated_by, updated_at,
                commit_date,cause_code,cause_code_remark,
                system_commit1_accepted,system_commit2_accepted,system_commit3_accepted,system_commit4_accepted,target_recovery]
                print("record inserted to advance_commit table with ori_part_number", ori_part_number, "prod_week and prod_year",prod_week, prod_year,"for demand category",demand_category)
                condb(sql, col)
                sql1 = """update forecast_plan set is_processed=1,active=1 where ori_part_number=%s and prod_week={} and prod_year={}""".format(prod_week, prod_year)
                col1 = [ori_part_number]
                ...
                (row.system_commit3==0 and row.system_commit3_date is None) and (row.system_commit4==0 and row.system_commit4_date is None):
                    ...
                    where ori_part_number=%s and prod_year={} and prod_week={} and process_week={} and demand_category='{}'""".format(new_commit1,new_commit2,new_commit3,new_commit4,prod_year,prod_week,process_week,demand_category)
                    print("record updated to advance_commit table with ori_part_number", ori_part_number,"prod_week and prod_year", prod_week, prod_year,"for demand category",demand_category)
                    col = [commit1_date,commit1_week,commit1_year,system_commit1_accepted,commit2_date,commit2_week,commit2_year,system_commit2_accepted,\
                    commit3_date,commit3_week,commit3_year,system_commit3_accepted,commit4_date,commit4_week,commit4_year,system_commit4_accepted,\
                    commit_date,cause_code,cause_code_remark,target_recovery,ori_part_number]
                    print(sql)
                    print(commit1_date,commit2_date,commit3_date,commit4_date)
                    condb(sql,col)
