...
def lambda_handler(event, context):
    ...
    print(start_time)
    print(next_time)
    phase=['COMMIT','REVIEW','CLOSED','PLANNING','NOT STARTED']
    s3_client = boto3.client('s3')
    df_json={
        "last_processed_at":str(start_time),
        "Status":"In Progress",
        "Next_process_starts_at":" "
            }
    json_body=json.dumps(df_json)
    s3_client.put_object(Body=json_body,Bucket=S3_BUCKET,Key='irp_event_timestamp/irp_processing_timestamp_prod.json')
    ...
    def get_phase(supplier_id):
        get_phase=URL_DOMAIN+'/api/forecast-cm-calendar-phases/supplier/{supplier_id}/week/{week}/year/{year}'.format(supplier_id=supplier_id,week=week_num,year=current_year)
    ...
    def handling_null(demand_category,ori_part_number,prod_week,prod_year):
        ...
        try:
            irp_value="select " + demand_category + "_advance_commit_irp from {}.forecast_plan where prod_week={} and prod_year={} and ori_part_number=%s".format(DB_NAME,commit_week,commit_year)
            col=[ori_part_number]
            advance_commit_irp=condb(irp_value,col)
            print(advance_commit_irp)
            if advance_commit_irp[0][0]==None:
                update_irp="update {}.forecast_plan set " +demand_category+"_advance_commit_irp=0 where prod_week={} and prod_year={} and ori_part_number=%s".format(DB_NAME,commit_week,commit_year)
                col=[ori_part_number]  
    ...
    def process_type_calculation(ori_part_number,commit_week,commit_year):
        ...
        FROM {}.forecast_plan where ori_part_number=%s and prod_week={} and prod_year={};""".format(DB_NAME,commit_week,commit_year)
        col=[ori_part_number]
    ...
    def process(current_phase, supplier_name):
        if current_phase in phase:
            ...
            primary_advance_commit_query="""select * from {}.advance_commit where process_week={} and (irp_processed=0 or irp_processed is null) and source_supplier='{}';""".format(DB_NAME,week_num,supplier_name)
            ...
            for index,primary in primary_advance_commit_data.iterrows():
                ...
                test="""select * from {}.advance_commit_backup where id={}""".format(DB_NAME,id)
                df_test=condb(test)
                ...
                if id_found!=[]:
                        ...
                        if id==s_id:
                            ...
                            if demand_category=='Forecast' and s_demand_category=='Forecast':
                                ...
                                if (is_delete==1):
                                    ...
                                    if s_commit1_accepted==1:
                                        ...
                                        sql="""update {}.forecast_plan set forecast_advance_commit_irp=(forecast_advance_commit_irp)-({}),updated_by=%s,updated_at=%s,active=1
                                        where ori_part_number=%s and prod_week={} and prod_year={}""".format(DB_NAME,s_commit1,s_commit1_week,s_commit1_year)
                                    ...
                                    if s_commit2_accepted==1:
                                        ...
                                        sql="""update {}.forecast_plan set forecast_advance_commit_irp=(forecast_advance_commit_irp)-({}),updated_by=%s,updated_at=%s,active=1
                                        where ori_part_number=%s and prod_week={} and prod_year={}""".format(DB_NAME,s_commit2,s_commit2_week,s_commit2_year)
                                        col=[updated_by,current_time,ori_part_number]
                                        condb(sql,col)
                                    ...
                                    if s_commit3_accepted==1:
                                        ...
                                        sql="""update {}.forecast_plan set forecast_advance_commit_irp=(forecast_advance_commit_irp)-({}),updated_by=%s,updated_at=%s,active=1
                                        where ori_part_number=%s and prod_week={} and prod_year={}""".format(DB_NAME,s_commit3,s_commit3_week,s_commit3_year)
                                        col=[updated_by,current_time,ori_part_number]
                                        condb(sql,col)
                                    ...
                                    if s_commit4_accepted==1:
                                        ...
                                        sql="""update {}.forecast_plan set forecast_advance_commit_irp=(forecast_advance_commit_irp)-({}),updated_by=%s,updated_at=%s,active=1
                                        where ori_part_number=%s and prod_week={} and prod_year={}""".format(DB_NAME,s_commit4,s_commit4_week,s_commit4_year)
                                        col=[updated_by,current_time,ori_part_number]
                                        condb(sql,col)
                                        process_type_calculation(ori_part_number,commit4_week,commit4_year)
                                    insert_delete="""insert into {}.advance_commit_deleted_records select * from {}.advance_commit where id={}""".format(DB_NAME,DB_NAME,id)
                                    condb(insert_delete)
                                    print("inserted the deleted record to deleted records table for id:",id)
                                    delete_record="""delete from {}.advance_commit where id={}""".format(DB_NAME,id)
                                    condb(delete_record)
                                ...
                                if (commit1 is not None)and(commit1_date==s_commit1_date):
                                    ...
                                    if (commit1!=s_commit1):
                                        ...
                                        if (commit1_accepted==1):
                                            ...
                                            handling_null(demand_category,ori_part_number,commit1_week,commit1_year)
                                            sql="""update {}.forecast_plan set forecast_advance_commit_irp=(forecast_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                                            where ori_part_number=%s and prod_week={} and prod_year={}""".format(DB_NAME,commit1,commit1_week,commit1_year)
                                            col=[updated_by,current_time,ori_part_number] 
                                            condb(sql,col)
                                            sql1="""update {}.advance_commit set irp_processed=1,commit1_irp_processed=0,planner_review_updated=0 where id={}""".format(DB_NAME,id)
                                            condb(sql1)
                                            process_type_calculation(ori_part_number,commit1_week,commit1_year)
                                    if (commit1==s_commit1):
                                ...
                                if (commit2 is not None)and(commit2_date==s_commit2_date):
                                    ...
                                    if (commit2!=s_commit2):
                                        ...
                                        if (commit2_accepted==1):
                                            ...
                                            handling_null(demand_category,ori_part_number,commit2_week,commit2_year)
                                            sql="""update {}.forecast_plan set forecast_advance_commit_irp=(forecast_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                                            where ori_part_number=%s and prod_week={} and prod_year={}""".format(DB_NAME,commit2,commit2_week,commit2_year)
                                            col=[updated_by,current_time,ori_part_number] 
                                            condb(sql,col)
                                            sql1="""update {}.advance_commit set irp_processed=1,commit2_irp_processed=0,planner_review_updated=0 where id={}""".format(DB_NAME,id)
                                            condb(sql1)
                                            process_type_calculation(ori_part_number,commit2_week,commit2_year)
                                    if (commit2==s_commit2):
                                ...
                                if (commit3 is not None)and(commit3_date==s_commit3_date):
                                    ...
                                    if (commit3!=s_commit3):
                                        ...
                                        if (commit3_accepted==1):
                                            ...
                                            handling_null(demand_category,ori_part_number,commit3_week,commit3_year)
                                            sql="""update {}.forecast_plan set forecast_advance_commit_irp=(forecast_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                                            where ori_part_number=%s and prod_week={} and prod_year={}""".format(DB_NAME,commit3,commit3_week,commit3_year)
                                            col=[updated_by,current_time,ori_part_number] 
                                            condb(sql,col)
                                            sql1="""update {}.advance_commit set irp_processed=1,commit3_irp_processed=0,planner_review_updated=0 where id={}""".format(DB_NAME,id)
                                            condb(sql1)
                                            process_type_calculation(ori_part_number,commit3_week,commit3_year)
                                    if (commit3==s_commit3):
                                ...
                                if (commit4 is not None)and(commit4_date==s_commit4_date):
                                    ...
                                    if (commit4!=s_commit4):
                                        ...
                                        if (commit4_accepted==1):
                                            ...
                                            handling_null(demand_category,ori_part_number,commit4_week,commit4_year)
                                            sql="""update {}.forecast_plan set forecast_advance_commit_irp=(forecast_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                                            where ori_part_number=%s and prod_week={} and prod_year={}""".format(DB_NAME,commit4,commit4_week,commit4_year)
                                            col=[updated_by,current_time,ori_part_number] 
                                            condb(sql,col)
                                            sql1="""update {}.advance_commit set irp_processed=1,commit4_irp_processed=0,planner_review_updated=0 where id={}""".format(DB_NAME,id)
                                            condb(sql1)
                                            process_type_calculation(ori_part_number,commit4_week,commit4_year)
                                    if (commit4==s_commit4):
                            ...
                            if demand_category=='GSA' and s_demand_category=='GSA':
                                ...
                                if (is_delete==1):
                                    ...
                                    if s_commit1_accepted==1:
                                        handling_null(demand_category,ori_part_number,commit1_week,commit1_year)
                                        sql="""update {}.forecast_plan set gsa_advance_commit_irp=(gsa_advance_commit_irp)-({}),updated_by=%s,updated_at=%s,active=1
                                        where ori_part_number=%s and prod_week={} and prod_year={}""".format(DB_NAME,s_commit1,s_commit1_week,s_commit1_year)
                                        col=[updated_by,current_time,ori_part_number]
                                        condb(sql,col)
                                        process_type_calculation(ori_part_number,s_commit1_week,s_commit1_year)
                            ...
                            if demand_category=='Non GSA' and s_demand_category=='Non GSA':
                                ...
                                if (is_delete==1):
                                    ...
                                    if s_commit1_accepted==1:
                                        handling_null(demand_category,ori_part_number,commit1_week,commit1_year)
                                        sql="""update {}.forecast_plan set nongsa_advance_commit_irp=(nongsa_advance_commit_irp)-({}),updated_by=%s,updated_at=%s,active=1
                                        where ori_part_number=%s and prod_week={} and prod_year={}""".format(DB_NAME,s_commit1,s_commit1_week,s_commit1_year)
                                        col=[updated_by,current_time,ori_part_number]
                                        condb(sql,col)
                                        process_type_calculation(ori_part_number,commit1_week,commit1_year)
                            ...
                            if demand_category=='System' and s_demand_category=='System':
                                ...
                                if (is_delete==1):
                                    ...
                                    if s_commit1_accepted==1:
                                        handling_null(demand_category,ori_part_number,commit1_week,commit1_year)
                                        sql="""update {}.forecast_plan set system_advance_commit_irp=(system_advance_commit_irp)-({}),updated_by=%s,updated_at=%s,active=1
                                        where ori_part_number=%s and prod_week={} and prod_year={}""".format(DB_NAME,s_commit1,s_commit1_week,s_commit1_year)
                                        col=[updated_by,current_time,ori_part_number]
                                        condb(sql,col)
                                        process_type_calculation(ori_part_number,s_commit1_week,s_commit1_year)
                    ...
                    else_id=primary.id
                    print("else",else_id)
                    test_primary="""select * from {}.advance_commit where id={}""".format(DB_NAME, else_id)
            ...
            time.sleep(10)
            cleaning_the_secondary_table="""delete from {}.advance_commit_backup where source_supplier='{}' and process_week={}""".format(DB_NAME, supplier_name, week_num)
            condb(cleaning_the_secondary_table)
            loading_the_latest_records="""insert into {}.advance_commit_backup select * from {}.advance_commit where process_week={} and source_supplier='{}'""".format(DB_NAME, DB_NAME, week_num, supplier_name)
    ...
    for supplier in suppliers:
        if supplier:
            ...
            print(supplier_name)
            current_phase = get_phase(supplier_id)
            process(current_phase, supplier_name)  
        else:
