            ...
            for s_index,secondary in secondary_advance_commit_data.iterrows():
                ...
                if id==s_id:
                    ...
                    if demand_category=='Forecast' and s_demand_category=='Forecast':
                        ...
                        def forecast_process_type(commit_week,commit_year):
                            try:
                                print("entered exception calculation")
                                exception_checking="""SELECT forecast_original,forecast_adj,gsa_supply,forecast_advance_commit_irp,id,is_dmp FROM User_App.forecast_plan 
                                where ori_part_number=%s and prod_week={} and prod_year={};""".format(commit_week,commit_year)
                                col=[ori_part_number]
                                df2=condb(exception_checking,col)
                                df1=list(sum(df2,()))
                                forecast_original=df1[0] if df1[0] is not None else 0
                                forecast_adj=df1[1] if df1[1] is not None else 0
                                forecast_supply=df1[2] if df1[2] is not None else 0
                                forecast_advance_commit_irp=df1[3] if df1[3] is not None else 0
                                id=df1[4]
                                is_dmp=1 if ord(df1[5])==1 else 0
                                process_type='Exception' if (((forecast_original+forecast_adj)-(forecast_supply+forecast_advance_commit_irp))!=0 and is_dmp==1) else None
                                print(forecast_original,forecast_adj,forecast_supply,forecast_advance_commit_irp,process_type,is_dmp)
                                sql="update User_App.forecast_plan set process_type=%s where id=%s"
                                col=[process_type,id]
                                print(sql,process_type,id)
                                condb(sql,col)
                            except:
                                print("process_type exception occurred")
                                pass
                            return print("process_type processed for",ori_part_number,process_type,commit_week,commit_year)
                        
                        if (is_delete==1)and(commit1_accepted==1):
                            print("deleted record found",is_delete)
                            ...
                            condb(sql,col)
                            forecast_process_type(commit1_week,commit1_year)
                        ...
                        if (commit1 is not None)and(commit1_date==s_commit1_date):
                            ...
                            if (commit1!=s_commit1):
                                ...
                                if (s_commit1_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    forecast_process_type(s_commit1_week,s_commit1_year)
                            ...
                            if (commit1==s_commit1):
                                ...
                                if (commit1_accepted==1)and(s_commit1_accepted==0):
                                    ...
                                    handling_null(demand_category,ori_part_number,commit1_week,commit1_year)
                                    update="""update User_App.forecast_plan set forecast_advance_commit_irp=(forecast_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                                    where ori_part_number=%s and prod_week={} and prod_year={}""".format(commit1,commit1_week,commit1_year)
                                    col=[updated_by,current_time,ori_part_number] 
                                    condb(update,col)
                                    sql1="""update User_App.advance_commit set irp_processed=1,commit1_irp_processed=0,planner_review_updated=0 where id={}""".format(id)
                        ...
                        if (is_delete==1)and(commit2_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            forecast_process_type(commit2_week,commit2_year)
                        ...
                        if (commit2 is not None)and(commit2_date==s_commit2_date):
                            ...
                            if (commit2!=s_commit2):
                                ...
                                if (s_commit2_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    forecast_process_type(s_commit2_week,s_commit2_year)
                        ...
                        if (is_delete==1)and(commit3_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            forecast_process_type(commit3_week,commit3_year)
                        ...
                        if (commit3 is not None)and(commit3_date==s_commit3_date):
                            ...
                            if (commit3!=s_commit3):
                                ...
                                if (s_commit3_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    forecast_process_type(s_commit3_week,s_commit3_year)
                        ...
                        if (is_delete==1)and(commit4_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            forecast_process_type(commit4_week,commit4_year)
                        ...
                        if (commit4 is not None)and(commit4_date==s_commit4_date):
                            ...
                            if (commit4!=s_commit4):
                                ...
                                if (s_commit4_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    forecast_process_type(s_commit4_week,s_commit4_year)
                    ...
                    if demand_category=='GSA' and s_demand_category=='GSA':
                        print('Forecast',id,s_id,ori_part_number,s_ori_part_number,prod_week,s_prod_week,prod_year,s_prod_year)
                        def gsa_process_type(commit_week,commit_year):
                            try:
                                print("entered exception calculation")
                                exception_checking="""SELECT forecast_original,gsa_adj,gsa_supply,gsa_advance_commit_irp,id,is_dmp FROM User_App.forecast_plan 
                                where ori_part_number=%s and prod_week={} and prod_year={};""".format(commit_week,commit_year)
                                col=[ori_part_number]
                                df2=condb(exception_checking,col)
                                df1=list(sum(df2,()))
                                forecast_original=df1[0] if df1[0] is not None else 0
                                gsa_adj=df1[1] if df1[1] is not None else 0
                                forecast_supply=df1[2] if df1[2] is not None else 0
                                gsa_advance_commit_irp=df1[3] if df1[3] is not None else 0
                                id=df1[4]
                                is_dmp=1 if ord(df1[5])==1 else 0
                                process_type='Exception' if (((forecast_original+gsa_adj)-(forecast_supply+gsa_advance_commit_irp))!=0 and is_dmp==1) else None
                                print(forecast_original,gsa_adj,forecast_supply,gsa_advance_commit_irp,process_type,is_dmp)
                                sql="update User_App.forecast_plan set process_type=%s where id=%s"
                                col=[process_type,id]
                                print(sql,process_type,id)
                                condb(sql,col)
                        ...
                        if (is_delete==1)and(commit1_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            gsa_process_type(commit1_week,commit1_year)
                        ...
                        if (commit1 is not None)and(commit1_date==s_commit1_date):
                            ...
                            if (commit1!=s_commit1):
                                ...
                                if (s_commit1_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    gsa_process_type(s_commit1_week,s_commit1_year)
                                if (commit1_accepted==1):
                                    print("new commit is accepted")
                                    handling_null(demand_category,ori_part_number,commit1_week,commit1_year)
                                    sql="""update User_App.forecast_plan set gsa_advance_commit_irp=(gsa_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                        ...
                        if (is_delete==1)and(commit2_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            gsa_process_type(commit2_week,commit2_year)
                        ...
                        if (commit2 is not None)and(commit2_date==s_commit2_date):
                            ...
                            if (commit2!=s_commit2):
                                ...
                                if (s_commit2_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    gsa_process_type(s_commit2_week,s_commit2_year)
                                if (commit2_accepted==1):
                                    print("new commit is accepted")
                                    handling_null(demand_category,ori_part_number,commit2_week,commit2_year)
                                    sql="""update User_App.forecast_plan set gsa_advance_commit_irp=(gsa_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                        ...
                        if (is_delete==1)and(commit3_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            gsa_process_type(commit3_week,commit3_year)
                        ...
                        if (commit3 is not None)and(commit3_date==s_commit3_date):
                            ...
                            if (commit3!=s_commit3):
                                ...
                                if (s_commit3_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    gsa_process_type(s_commit3_week,s_commit3_year)
                                if (commit3_accepted==1):
                                    print("new commit is accepted")
                                    handling_null(demand_category,ori_part_number,commit3_week,commit3_year)
                                    sql="""update User_App.forecast_plan set gsa_advance_commit_irp=(gsa_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                        ...
                        if (is_delete==1)and(commit4_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            gsa_process_type(commit4_week,commit4_year)
                        ...
                        if (commit4 is not None)and(commit4_date==s_commit4_date):
                            ...
                            if (commit4!=s_commit4):
                                ...
                                if (s_commit4_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    gsa_process_type(s_commit4_week,s_commit4_year)
                                if (commit4_accepted==1):
                                    print("new commit is accepted")
                                    handling_null(demand_category,ori_part_number,commit4_week,commit4_year)
                                    sql="""update User_App.forecast_plan set gsa_advance_commit_irp=(gsa_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                    ...
                    if demand_category=='Non GSA' and s_demand_category=='Non GSA':
                        print('Forecast',id,s_id,ori_part_number,s_ori_part_number,prod_week,s_prod_week,prod_year,s_prod_year)
                        def nongsa_process_type(commit_week,commit_year):
                            try:
                                print("entered exception calculation")
                                exception_checking="""SELECT forecast_original,nongsa_adj,nongsa_supply,nongsa_advance_commit_irp,id,is_dmp FROM User_App.forecast_plan 
                                where ori_part_number=%s and prod_week={} and prod_year={};""".format(commit_week,commit_year)
                                col=[ori_part_number]
                                df2=condb(exception_checking,col)
                                df1=list(sum(df2,()))
                                forecast_original=df1[0] if df1[0] is not None else 0
                                nongsa_adj=df1[1] if df1[1] is not None else 0
                                forecast_supply=df1[2] if df1[2] is not None else 0
                                nongsa_advance_commit_irp=df1[3] if df1[3] is not None else 0
                                id=df1[4]
                                is_dmp=1 if ord(df1[5])==1 else 0
                                process_type='Exception' if (((forecast_original+nongsa_adj)-(forecast_supply+nongsa_advance_commit_irp))!=0 and is_dmp==1) else None
                                print(forecast_original,nongsa_adj,forecast_supply,nongsa_advance_commit_irp,process_type,is_dmp)
                                sql="update User_App.forecast_plan set process_type=%s where id=%s"
                                col=[process_type,id]
                                print(sql,process_type,id)
                                condb(sql,col)
                        ...
                        if (is_delete==1)and(commit1_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            nongsa_process_type(commit1_week,commit1_year)
                        ...
                        if (commit1 is not None)and(commit1_date==s_commit1_date):
                            ...
                            if (commit1!=s_commit1):
                                ...
                                if (s_commit1_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    nongsa_process_type(s_commit1_week,s_commit1_year)
                                if (commit1_accepted==1):
                                    print("new commit is accepted")
                                    handling_null(demand_category,ori_part_number,commit1_week,commit1_year)
                                    sql="""update User_App.forecast_plan set nongsa_advance_commit_irp=(nongsa_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                        ...
                        if (is_delete==1)and(commit2_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            nongsa_process_type(commit2_week,commit2_year)
                        ...
                        if (commit2 is not None)and(commit2_date==s_commit2_date):
                            ...
                            if (commit2!=s_commit2):
                                ...
                                if (s_commit2_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    nongsa_process_type(s_commit2_week,s_commit2_year)
                                if (commit2_accepted==1):
                                    print("new commit is accepted")
                                    handling_null(demand_category,ori_part_number,commit2_week,commit2_year)
                                    sql="""update User_App.forecast_plan set nongsa_advance_commit_irp=(nongsa_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                        ...
                        if (is_delete==1)and(commit3_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            nongsa_process_type(commit3_week,commit3_year)
                        ...
                        if (commit3 is not None)and(commit3_date==s_commit3_date):
                            ...
                            if (commit3!=s_commit3):
                                ...
                                if (s_commit3_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    nongsa_process_type(s_commit3_week,s_commit3_year)
                                if (commit3_accepted==1):
                                    print("new commit is accepted")
                                    handling_null(demand_category,ori_part_number,commit3_week,commit3_year)
                                    sql="""update User_App.forecast_plan set nongsa_advance_commit_irp=(nongsa_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                        ...
                        if (is_delete==1)and(commit4_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            nongsa_process_type(commit4_week,commit4_year)
                        ...
                        if (commit4 is not None)and(commit4_date==s_commit4_date):
                            ...
                            if (commit4!=s_commit4):
                                ...
                                if (s_commit4_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    nongsa_process_type(s_commit4_week,s_commit4_year)
                                if (commit4_accepted==1):
                                    print("new commit is accepted")
                                    handling_null(demand_category,ori_part_number,commit4_week,commit4_year)
                                    sql="""update User_App.forecast_plan set nongsa_advance_commit_irp=(nongsa_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                    ...
                    if demand_category=='System' and s_demand_category=='System':
                        print('Forecast',id,s_id,ori_part_number,s_ori_part_number,prod_week,s_prod_week,prod_year,s_prod_year)
                        def system_process_type(commit_week,commit_year):
                            try:
                                print("entered exception calculation")
                                exception_checking="""SELECT forecast_original,system_adj,system_supply,system_advance_commit_irp,id,is_dmp FROM User_App.forecast_plan 
                                where ori_part_number=%s and prod_week={} and prod_year={};""".format(commit_week,commit_year)
                                col=[ori_part_number]
                                df2=condb(exception_checking,col)
                                df1=list(sum(df2,()))
                                forecast_original=df1[0] if df1[0] is not None else 0
                                system_adj=df1[1] if df1[1] is not None else 0
                                forecast_supply=df1[2] if df1[2] is not None else 0
                                system_advance_commit_irp=df1[3] if df1[3] is not None else 0
                                id=df1[4]
                                is_dmp=1 if ord(df1[5])==1 else 0
                                process_type='Exception' if (((forecast_original+system_adj)-(forecast_supply+system_advance_commit_irp))!=0 and is_dmp==1) else None
                                print(forecast_original,system_adj,forecast_supply,system_advance_commit_irp,process_type,is_dmp)
                                sql="update User_App.forecast_plan set process_type=%s where id=%s"
                                col=[process_type,id]
                                print(sql,process_type,id)
                                condb(sql,col)
                        ...
                        if (is_delete==1)and(commit1_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            system_process_type(commit1_week,commit1_year)
                        ...
                        if (commit1 is not None)and(commit1_date==s_commit1_date):
                            ...
                            if (commit1!=s_commit1):
                                ...
                                if (s_commit1_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    system_process_type(s_commit1_week,s_commit1_year)
                                if (commit1_accepted==1):
                                    print("new commit is accepted")
                                    handling_null(demand_category,ori_part_number,commit1_week,commit1_year)
                                    sql="""update User_App.forecast_plan set system_advance_commit_irp=(system_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                        ...
                        if (is_delete==1)and(commit2_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            system_process_type(commit2_week,commit2_year)
                        ...
                        if (commit2 is not None)and(commit2_date==s_commit2_date):
                            ...
                            if (commit2!=s_commit2):
                                ...
                                if (s_commit2_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    system_process_type(s_commit2_week,s_commit2_year)
                                if (commit2_accepted==1):
                                    print("new commit is accepted")
                                    handling_null(demand_category,ori_part_number,commit2_week,commit2_year)
                                    sql="""update User_App.forecast_plan set system_advance_commit_irp=(system_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                        ...
                        if (is_delete==1)and(commit3_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            system_process_type(commit3_week,commit3_year)
                        ...
                        if (commit3 is not None)and(commit3_date==s_commit3_date):
                            ...
                            if (commit3!=s_commit3):
                                ...
                                if (s_commit3_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    system_process_type(s_commit3_week,s_commit3_year)
                                if (commit3_accepted==1):
                                    print("new commit is accepted")
                                    handling_null(demand_category,ori_part_number,commit3_week,commit3_year)
                                    sql="""update User_App.forecast_plan set system_advance_commit_irp=(system_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
                        ...
                        if (is_delete==1)and(commit4_accepted==1):
                            ...
                            col=[updated_by,current_time,ori_part_number]
                            condb(sql,col)
                            system_process_type(commit4_week,commit4_year)
                        ...
                        if (commit4 is not None)and(commit4_date==s_commit4_date):
                            ...
                            if (commit4!=s_commit4):
                                ...
                                if (s_commit4_accepted==1):
                                    ...
                                    col=[updated_by,current_time,ori_part_number]
                                    condb(sql,col)
                                    system_process_type(s_commit4_week,s_commit4_year)
                                if (commit4_accepted==1):
                                    print("new commit is accepted")
                                    handling_null(demand_category,ori_part_number,commit4_week,commit4_year)
                                    sql="""update User_App.forecast_plan set system_advance_commit_irp=(system_advance_commit_irp)+({}),updated_by=%s,updated_at=%s,active=1
