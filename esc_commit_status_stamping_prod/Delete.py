...
def delete_functionality():
    ...
    primary_advance_commit_query="""select * from advance_commit where process_week={} and is_delete=1 and irp_processed=0""".format(week_num)
    ...
    def handling_null(demand_category,ori_part_number,prod_week,prod_year):
            ...
            try:
                irp_value="select " + demand_category + "_advance_commit_irp from forecast_plan where prod_week={} and prod_year={} and ori_part_number=%s".format(commit_week,commit_year)
                col=[ori_part_number]
                advance_commit_irp=condb(irp_value,col)
                print(advance_commit_irp)
                if advance_commit_irp[0][0]==None:
                    update_irp="update forecast_plan set " +demand_category+"_advance_commit_irp=0 where prod_week={} and prod_year={} and ori_part_number=%s".format(commit_week,commit_year)
                    col=[ori_part_number]  
    ...
    def process_type_calculation(ori_part_number,commit_week,commit_year):
        ...
        FROM forecast_plan where ori_part_number=%s and prod_week={} and prod_year={};""".format(commit_week,commit_year)
        col=[ori_part_number]
        ...
        sql="update forecast_plan set process_type=%s where id=%s"
        col=[process_type,id]
    ...
    for index,primary in primary_advance_commit_data.iterrows():
        ...
        if demand_category=='Forecast':
            print("deleted record found",id,demand_category,ori_part_number,commit1_week,commit1_year)
        ...
        insert_delete="""insert into advance_commit_deleted_records select * from advance_commit where id={}""".format(id)
        condb(insert_delete)
        print("inserted the deleted record to deleted records table for id:",id)
        delete_record="""delete from advance_commit where id={}""".format(id)
        condb(delete_record)
