...
def lambda_handler(event,context):
    ...
    file=event['Records'][0]['s3']['object']['key'][19:].replace('+',' ').replace('%28','(').replace('%29',')').replace('%3A',':')
    ...
    BODY_TEXT = "Hello,\n\r\nPlease find the attached file for the Escalation Summary pending for your actions.\n\nThanks & Regards,\nKeysightIT-Team\n\n\n"
    CHARSET = "utf-8"
    ...
    s3_object = s3_object.get_object(Bucket=bucket_name, Key='Escalation_Summary/'+file)
    body = s3_object['Body'].read()
    # print(body) 
    file_name=file
    msg = MIMEMultipart('mixed')
    msg['Subject']="Open Escalation Summary for {} {}".format(supplier_short_name, sng_time)
    msg['From']=SENDER
