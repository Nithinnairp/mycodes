...
def lambda_handler(event, context):
    ...
    def validate_kr_summary(source_supplier, source_supplier_id):
        ...
        Validate_KR_Summary_Report_Commit_Value="""select count(*) as count1
            ...
            where
            (((ifnull(curr_mon.nongsa_supply,0)+ifnull(curr_mon.cumulative_nongsa_advance_commit_irp,0))!=
            ((ifnull(last_mon.nongsa_supply,0)+ifnull(last_mon.cumulative_nongsa_advance_commit_irp,0))+
            (ifnull(fri.nongsa_commit,0)+ifnull(fri.nongsa_advance_commit_irp,0)))) or
            ((ifnull(curr_mon.gsa_supply,0)+ifnull(curr_mon.cumulative_gsa_advance_commit_irp,0))!=
            ((ifnull(last_mon.gsa_supply,0)+ifnull(last_mon.cumulative_gsa_advance_commit_irp,0))+
            (ifnull(fri.gsa_commit,0)+ifnull(fri.gsa_advance_commit_irp,0)))) or
            ((ifnull(curr_mon.forecast_supply,0)+ifnull(curr_mon.cumulative_forecast_advance_commit_irp,0))!=
            ((ifnull(last_mon.forecast_supply,0)+ifnull(last_mon.cumulative_forecast_advance_commit_irp,0))+
            (ifnull(fri.forecast_commit,0)+ifnull(fri.forecast_advance_commit_irp,0)))) or
            ((ifnull(curr_mon.system_supply,0)+ifnull(curr_mon.cumulative_system_advance_commit_irp,0))!=
            ((ifnull(last_mon.system_supply,0)+ifnull(last_mon.cumulative_system_advance_commit_irp,0))+
            (ifnull(fri.system_commit,0)+ifnull(fri.system_advance_commit_irp,0)))))
            and
            ...
            (curr_mon.prod_week between curr_mon.process_week and curr_mon.process_week+8) and
        ...
        if df1['count1']==0:
            ...
            response = client.send_email(Source=SENDER,Destination={'ToAddresses':RECIPIENT},
                            Message={
                                'Subject': {
                                    'Data': "Validation of KR Summary Report Commit Value Result for {} WW-{}".format(supplier_short_name, week_num)
                                },
                                'Body': {
                                    'Text': {
                                        'Data': """Hello,\n\r\nNo errors were found in KR Summary Report Commit Value as of {} in first 8 weeks.\n\nValidation Details:-'Validate KR Summary Report Commit Value = Past Mon Cumm commit + Past Fri new commit inc adv commit YES by item by dmd category'\n\nThanks & Regards,\nKeysightIT-Team\n\n\n""".format(sng_time)
                                    }
                                }
                            })
        else:
            ...
            df3=pd.DataFrame(data=result,columns=col)
            ...
            response = client.send_email(Source=SENDER,Destination={'ToAddresses':RECIPIENT},
                            Message={
                                ...
                                'Body': {
                                    'Text': {
                                        'Data': """Hello,\n\r\nErrors were found in KR Summary Report Commit Value as of {} in first 8 weeks, download the file from below url.\nURL- {}
    ...
    for supplier in suppliers:
        if supplier:
            ...
            validate_kr_summary(supp_name, supp_id)
        else:
